...
tier4_external_api_msgs__msg__Emergency__create()
{
  ...
  tier4_external_api_msgs__msg__Emergency * msg = (tier4_external_api_msgs__msg__Emergency *)allocator.allocate(sizeof(tier4_external_api_msgs__msg__Emergency), allocator.state);
  if (!msg) {
    return NULL;
  }
...
tier4_external_api_msgs__msg__Emergency__Sequence__init(tier4_external_api_msgs__msg__Emergency__Sequence * array, size_t size)
{
  ...
  if (size) {
    data = (tier4_external_api_msgs__msg__Emergency *)allocator.zero_allocate(size, sizeof(tier4_external_api_msgs__msg__Emergency), allocator.state);
    if (!data) {
      return false;
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  tier4_external_api_msgs__msg__Emergency__Sequence * array = (tier4_external_api_msgs__msg__Emergency__Sequence *)allocator.allocate(sizeof(tier4_external_api_msgs__msg__Emergency__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  ...
  tier4_external_api_msgs__msg__Emergency__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tier4_external_api_msgs__msg__Emergency * data =
      (tier4_external_api_msgs__msg__Emergency *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tier4_external_api_msgs__msg__Emergency__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tier4_external_api_msgs__msg__Emergency__fini(&output->data[i]);
        }
        return false;
      }
    }
