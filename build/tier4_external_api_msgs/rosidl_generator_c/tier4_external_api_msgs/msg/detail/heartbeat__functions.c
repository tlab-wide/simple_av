  ...
  tier4_external_api_msgs__msg__Heartbeat__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tier4_external_api_msgs__msg__Heartbeat * data =
      (tier4_external_api_msgs__msg__Heartbeat *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tier4_external_api_msgs__msg__Heartbeat__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tier4_external_api_msgs__msg__Heartbeat__fini(&output->data[i]);
        }
        return false;
      }
    }
