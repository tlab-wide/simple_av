  ...
  autoware_system_msgs__msg__HazardStatusStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_system_msgs__msg__HazardStatusStamped * data =
      (autoware_system_msgs__msg__HazardStatusStamped *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_system_msgs__msg__HazardStatusStamped__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          autoware_system_msgs__msg__HazardStatusStamped__fini(&output->data[i]);
        }
        return false;
      }
    }
